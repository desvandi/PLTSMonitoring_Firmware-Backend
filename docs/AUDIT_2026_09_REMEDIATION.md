...
| P1-6 | OTA modular observability → GAS | `MqttTelemetryPublisher::publishOtaLifecycle()` emits ACCEPTED/DOWNLOADING/VERIFIED/FLASHED/ACTIVATED/ROLLBACK/FAILED on `plts/<deviceId>/ota/event`. [audit p.432 correction] Transport is PubSubClient QoS 0 (fire-and-forget socket write — PubSubClient 2.8 cannot publish above QoS 0); delivery is at-least-once via spool replay, NOT broker-acknowledged. `OtaManager` calls it at every transition. | Code review. GAS dedupes on (deviceId, jobId, state). |
