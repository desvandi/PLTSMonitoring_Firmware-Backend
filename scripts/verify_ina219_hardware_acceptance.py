...
def check_hardware_identity(hw, blockers, warnings, version):
    ...
    if not serial:
        ...
        hw_serial = str(hw.get("hardwareSerial", "") or "").strip()
...
def run() -> int:
    ...
    # [Audit 2026-09-05 re-audit, sections 17+20] Evidence for versions
    # >= 1.9.3 must bind the HARDWARE identity in addition to the firmware
    # identity: boardRevision (validated against the hardware revision
    # registry — DEVELOPMENT-ONLY PCB revisions are BLOCKED as an acceptance
    # basis), deviceSerial (must match the top-level hardwareSerial field),
    # and relayBoardRevision. This makes the acceptance boundary between the
    # v1.9.3 software release line and the S12/S10 PCB development work
    # EXPLICIT and machine-enforced. Making the evidence schema STRICTER is
    # intentional: this gate must never be weakened to pass wrong evidence.
    check_hardware_identity(hw, blockers, warnings, args.version)
