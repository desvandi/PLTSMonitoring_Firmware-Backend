...
// ---------------------------------------------------------------------------
static constexpr const char* FIRMWARE_VERSION        = "1.9.3";  // v1.9.3: reproducible build release — closes REL-03/REL-04 (build identity is now deterministic: same source commit + same pinned toolchain = byte-identical SHA-256; buildDate reports the SOURCE commit date, not the wall-clock compile time). Measurement chain is unchanged from v1.9.2: INA219 dynamic gain switching (canonical register fix), PGA ±80mV/±160mV with hysteresis 90A/100A, 190k/10k voltage divider (20:1), pga_mode telemetry, 0x0FFF/0x17FF per TI SBOS448G.

// [REL-03/REL-04 v1.9.3] Deterministic build identity.
// Before v1.9.3 this was `__DATE__ " " __TIME__` (wall-clock of the compile),
// which made every build byte-different → different SHA-256 for the same
// source commit (REL-03: hw-tested binary ≠ released binary). The build date
// is now derived from SOURCE_DATE_EPOCH (reproducible-builds.org convention):
// firmware/scripts/set_build_date.py resolves it from the git HEAD commit
// timestamp and generates .pio/generated/plts_build_date.h with the ISO-8601
// string below. Scriptless builds (e.g. Arduino IDE) fall back to a constant
// epoch string — still deterministic, never a wall clock.
#if __has_include("plts_build_date.h")
#include "plts_build_date.h"   // generated by firmware/scripts/set_build_date.py
#endif
#ifndef PLTS_BUILD_DATE_STR
#define PLTS_BUILD_DATE_STR "1970-01-01T00:00:00Z"
#endif
static constexpr const char* FIRMWARE_BUILD_DATE     = PLTS_BUILD_DATE_STR;  // semantic change: SOURCE commit date (deterministic), not compile wall-clock
...
// ---------------------------------------------------------------------------
// [CI fix] Include generated production secrets header BEFORE the #error guards
// so PEM macros (MQTT_ROOT_CA, OTA_HTTPS_ROOT_CA) are defined before the guards
// check for them. The header is auto-generated by inject_production_flags.py.
#if __has_include(<production_secrets.h>)
#include <production_secrets.h>
#endif
...
// [CI fix] Include generated production secrets header if it exists.
// This file is auto-generated by scripts/inject_production_flags.py and
// contains PEM certificates (multiline values that can't go on the -D
// command line). It's in .pio/generated/ which is added to CPPPATH.
// NOTE: The __has_include at line 435 (before #error guards) handles the
// actual include. This comment block remains for documentation.
